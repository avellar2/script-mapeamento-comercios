#!/usr/bin/env python3
"""
Testa apenas Dra Isadora Correa com o novo codigo de fechamento de modal.
...
logger = logging.getLogger("test-isadora")

PROFILE_DIR = Path(__file__).resolve().parent.parent / "profiles" / "whatsapp_match"
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "output" / "avgestao" / "test-isadora"
CAMPAIGN_KEY = "avgestao:assistencias:primeiro_contato:v1"

LEAD_ID = "42e016c6-0f7f-4e93-a98f-35fa0760420c"
LEAD_NOME = "Dra Isadora Correa Sant'anna"
LEAD_TEL = "+55 21 3745-8668"
...
async def run():
    ...
    phone_normalized = normalizar_telefone_br(LEAD_TEL)
    logger.info("Telefone normalizado: %s", phone_normalized)
    ...
    logger.info("=" * 50)
    logger.info("Testando: %s (%s)", LEAD_NOME, LEAD_TEL)
    logger.info("=" * 50)

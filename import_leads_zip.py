...
def _mostrar_dry_run(resultado: dict, existentes: dict, amostra: int = 5) -> None:
    ...
    for lead in resultado['validos'][:amostra]:
        ...
        print(f"    {lead.get('nome', '?')[:35]:35s} | {masked:14s} | {lead.get('subnicho', '?'):15s} | {lead.get('cidade', '?')}")
...
def importar_leads_zip(zip_path: str, produto: str = "avgestao", grupo: str = "assistencias",
    ...
    if todos_tels:
        try:
            ...
            logger.warning("Nao foi possivel consultar Supabase: %s. Prosseguindo sem dedup remota.", e)

    # Dry-run
    if dry_run or not confirm:
        _mostrar_dry_run(resultado, existentes)
        return 0

...
export default function FloatingWhatsAppButton({ data }: { data: ClientData }) {
  const [isVisible, setIsVisible] = useState(false);
  const [showTooltip, setShowTooltip] = useState(false);
  const waLink = createWhatsAppLink(data.whatsapp, data.businessName);
  const tooltipText = `Olá! Quer saber mais sobre ${data.businessName}? Clique aqui.`;
      ...
      {isVisible && (
        ...
        >
          <AnimatePresence>
            {showTooltip && (
              ...
              >
                ...
                <p className="text-xs text-slate-600 pr-4">{tooltipText}</p>
          ...
          <a
            ...
            aria-label={`Contatar ${data.businessName} pelo WhatsApp`}
          >

...
**AVGESTÃO** é a prioridade — LP estava difícil de vender, pivotou pro sistema de gestão.
- Envio automático: `enviar_auto_avgestao.py` — Playwright, perfil `.whatsapp_business_profile`, 7min entre msgs, sem pausa de bloco (vai direto)
- headless=False (NUNCA mudar sem perguntar)
- 23/06: 38 leads enviados, 1 interessado
- 27/06: 28 leads de assistência técnica importados e enviando
- OpenCode + glm-5.2 usado pra reescrever script de envio automático

Use esta skill pra escolher modelo. Para campanha AVGESTÃO, use `campanha-avgestao`. Para LP + tráfego, use `local-sales-prospecting`.